...
if (!defined('PGSQL'))  define('PGSQL', 'PGSQL');
// SQLite intentionally disabled
...
if ($DBMS === 'MySQL' || $DBMS === 'PGSQL') {
    $DBMS = htmlspecialchars(strip_tags($DBMS));
} else {
    $DBMS = 'MySQL'; // force safe default
}
...
function dvwaDatabaseConnect() {
	...
	if (isset($_DVWA['SQLI_DB']) && $_DVWA['SQLI_DB'] === 'SQLITE') {
		die('SQLite is not supported in this DVWA setup.');
